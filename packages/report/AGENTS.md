...
- Colour and type tokens live in `src/theme.css`. Verdict colours: Clear path `#047251`/`#07AE77`, Partial path `#A9611C`/`#D97230`, No path `#B23A2E`/`#C0392B`, Further review `#1F6FA8`/`#3E8DC4`.
...
- Two outbound requests only, both from `src/api.ts`, both user-initiated: the gate form (first name, last name, email, version, run id, lead source) and the meeting-request form (the same plus the report summary, the required `message`, and the optional `migration_timing`). The print control is not an outbound request.
